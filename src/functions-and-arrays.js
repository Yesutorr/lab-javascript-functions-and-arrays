...
function sum(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (typeof element === "number") {
      ...
      result += element ? 1 : 0;
    } else {throw new Error("tipo de caracter no valido");
    }
...
function averageWordLength(arrWords) {
 ...
 return sum (arrWords) / arrWords.length; ;
...
function uniquifyArray(arrayMix) {
  ...
  for (let i = 0; i < arrayMix.length; i++) {
    let wordsUnique = arrayMix[i];
    if (newArray.indexOf(wordsUnique) === -1) {
...
function doesWordExist(letterArray,searchedWord) {
  if (!letterArray.length) return null;
  return letterArray.indexOf(searchedWord) !== -1;
}
...
function howManyTimes(arra,palabraBuscada) {
  let count = 0;
  for (let i = 0; i <arra.length; i++) {
    if (arra[i] === palabraBuscada) {
      count++;
    }
  }
  return count;
